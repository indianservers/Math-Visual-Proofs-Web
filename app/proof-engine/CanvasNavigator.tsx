...
export function CanvasNavigator<Id extends string>({
  ...
  return (
    <section
      className={clsx(styles.navigator, !expanded && styles.collapsed)}
      aria-label="Easy canvas controller"
    >
      <div className={styles.topline}>
        <div>
          <b>{selected ? `${selected.label} is ready` : "Canvas controls"}</b>
      ...
      {expanded && helpOpen && (
        <div className={styles.help} role="note">
          <span>
            <b>1.</b> Pick a numbered shape.
          </span>
          <span>
            <b>2.</b> Drag it or use the big arrows.
          </span>
          <span>
            <b>3.</b> Rotate, then press Attach when it matches.
          </span>
        </div>
      )}
      ...
      {expanded && (
        <div className={styles.controls}>
          <div className={styles.picker} aria-label="Shapes on this canvas">
            {items.map((item) => (
              <button
                ...
                style={
                  {
                    "--piece-color": item.color ?? "#7351f5",
                  } as CSSProperties
                }
              >
                <i>
                  {item.complete ? "✓" : item.label.replace(/\D/g, "") || "◆"}
                </i>
          ...
          <div className={styles.actions}>
            {onRotate && (
              <button type="button" onClick={onRotate} disabled={!selected}>
                <b>↻</b> Rotate
              </button>
            )}
            {onDock && (
      ...
      <div
        ...
        aria-label={`${completedCount} of ${items.length} shapes attached`}
      >
