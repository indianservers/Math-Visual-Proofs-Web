...
export function InvariantIndicator({
  icon,
  label,
  complete,
}: {
  ...
  return (
    <div className={clsx("invariant-chip", complete && "complete")}>
...
export function ProofStepNavigator({
  labels,
  active,
}: {
  ...
  return (
    <div
      className="steps proof-step-nav"
      aria-label={`Proof step ${active + 1} of ${labels.length}`}
    >
      {labels.map((label, index) => (
        <span key={label} className="step-wrap">
          ...
          {index < labels.length - 1 && <span className="step-line" />}
        </span>
      ))}
    </div>
  );
...
const STARTS: Record<FormulaId, { x: number; y: number }> = {
  c2: { x: 18, y: 17 },
  equals: { x: 88, y: 17 },
  a2: { x: 158, y: 17 },
  plus: { x: 228, y: 17 },
...
export function FormulaDock({
  enabled,
  resetKey,
  animationFillCount = 0,
  onComplete,
}: {
  enabled: boolean;
  resetKey?: number;
  animationFillCount?: number;
  onComplete: () => void;
}) {
  ...
  const [message, setMessage] = useState(
    "Drag each area block into its matching dashed slot.",
  );
  const offset = useRef({ x: 28, y: 20 });
  const latest = useRef(positions);
  const completedNotified = useRef(false);
  const canvas = useProofCanvas<FormulaId>(
    760,
    118,
    (id, point) => {
      const next = {
        ...positions,
        [id]: { x: point.x - offset.current.x, y: point.y - offset.current.y },
      };
      latest.current = next;
      setPositions(next);
    },
    (id) => {
      ...
      if (Math.hypot(current.x - target.x, current.y - target.y) <= 48) {
        ...
        setMessage(
          `${FORMULA_LABELS[id].replace("^2", " squared")} connected to its visible region.`,
        );
      } else {
        ...
        setPositions(next);
        setMessage(
          "That block belongs in a different slot. It returned to its last valid place.",
        );
      }
    },
    { disabled: !enabled },
  ...
  useEffect(() => {
    if (placed.length !== FORMULA_ORDER.length || completedNotified.current)
      return;
  ...
  const animatedIds =
    animationFillCount > 0 ? FORMULA_ORDER.slice(0, animationFillCount) : [];
  ...
  return (
    <div className={clsx("formula-dock", !enabled && "locked")}>
      <div className="formula-dock-head">
        <b>Build the final equation</b>
        <span aria-live="polite">
          {enabled
            ? message
            : "Complete the rearrangement and compare the gaps first."}
        </span>
      </div>
      <svg
        viewBox="0 0 760 118"
        className={canvas.canvasClassName("formula-svg", "interactive-svg")}
        {...canvas.canvasProps}
        aria-label="Draggable equation block workspace"
      >
        <text x="18" y="108" className="formula-caption">
          AREA BLOCKS
        </text>
        {FORMULA_ORDER.map((id, index) => (
          <g key={`slot-${id}`}>
            <rect
              x={390 + index * 70}
              y="67"
              ...
              className={clsx(
                "formula-slot",
                visiblePlaced.includes(id) && "filled",
              )}
            />
            <text
              x={418 + index * 70}
              y="93"
              textAnchor="middle"
              className="formula-slot-hint"
            >
              {visiblePlaced.includes(id) ? "" : "?"}
            </text>
          </g>
        ))}
        {Object.keys(FORMULA_LABELS).map((key) => {
          const id = key as FormulaId;
          const pos = visiblePositions[id];
          return (
            <g
              key={id}
              ...
              className={clsx(
                ...
                visiblePlaced.includes(id) && "placed",
              )}
              onPointerDown={(event: PointerEvent<SVGGElement>) => {
                setSelected(id);
                canvas.beginDrag(id, event);
              }}
              tabIndex={enabled ? 0 : -1}
              role="button"
              aria-label={`Equation block ${FORMULA_LABELS[id]}`}
              onKeyDown={(event) => {
                if (event.key === "Enter" || event.key === " ")
                  placeByKeyboard(id);
              }}
            >
              <rect width="56" height="40" rx="9" />
              <foreignObject x="1" y="5" width="54" height="30">
                <div className="formula-block-math">
                  <MathFormula latex={FORMULA_LABELS[id]} />
                </div>
              </foreignObject>
            </g>
          );
